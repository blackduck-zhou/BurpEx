...
public class findName implements ScanCheck {
    ...
    public AuditResult passiveAudit(HttpRequestResponse var1){
        String p="(xsh|admin|guest|tianxx)";
        Pattern pattern_All=Pattern.compile(p,Pattern.CASE_INSENSITIVE);
        String background="Sensitive information found: Employee name!";
        ...
        List<AuditIssue> auditIssueList = marker_list.isEmpty() ? emptyList() : singletonList(
                AuditIssue.auditIssue("Sensitive information found:Employee name",
                        "Sensitive information found:Employee name!",
                        req.url(),
                        req.url(),
