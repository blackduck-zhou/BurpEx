...
public class findMail implements ScanCheck {
    @Override
    ...
    public AuditResult passiveAudit(HttpRequestResponse var1){
        String p="\\s*\\w+(?:\\.{0,1}[\\w-]+)*@[a-zA-Z0-9]+(?:[-.][a-zA-Z0-9]+)*\\.[a-zA-Z]+\\s*";
        Pattern pattern_All=Pattern.compile(p);
        String background="Sensitive information found:email address!";
        ...
        List<AuditIssue> auditIssueList = marker_list.isEmpty() ? emptyList() : singletonList(
                AuditIssue.auditIssue("Sensitive information found:email address!",
                        "Sensitive information found:email address!",
                        "Sensitive information found:email address!",
                        req.url(),
