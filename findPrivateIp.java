...
public class findPrivateIp implements ScanCheck {
    @Override
    ...
    public AuditResult passiveAudit(HttpRequestResponse var1){
        String p="(10|172|192)\\.\\d{1,3}\\.\\d{1,3}\\.\\d{1,3}";
        Pattern pattern_All=Pattern.compile(p);
        String background="Sensitive information found: Private IP!";
        ...
        List<AuditIssue> auditIssueList = marker_list.isEmpty() ? emptyList() : singletonList(
                AuditIssue.auditIssue("Sensitive information found:Private IP!",
                        "Sensitive information found:Private IP!",
                        "Sensitive information found:Private IP!",
                        req.url(),
